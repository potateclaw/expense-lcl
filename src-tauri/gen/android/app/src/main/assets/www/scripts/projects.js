...
async function fetchProjects() {
    try {
        ...
        projectsData.projects = await window.__invoke__('get_projects') || [];
    } catch (e) {
...
async function fetchProjectDetail(projectId) {
    try {
        ...
        const project = await window.__invoke__('get_projects');
        projectsData.currentProject = project.find(p => p.id === projectId) || null;

        if (projectsData.currentProject) {
            projectsData.categories = await window.__invoke__('get_project_categories', { projectId }) || [];
        } else {
...
async function suggestCategories() {
    ...
    try {
        const response = await window.__invoke__('chat_query', { query: `Suggest 4-6 spending categories for a project called "${projectName}". Reply with just a comma-separated list of category names, no description.` });
...
async function submitCreateProject() {
    ...
    try {
        const projectId = await window.__invoke__('create_project', {
            name,
...
async function submitAddCategory() {
    ...
    try {
        await window.__invoke__('add_project_category', {
...
async function submitEditProject() {
    ...
    try {
        await window.__invoke__('update_project', {
            id: currentProjectId,
            name,
            budget: Math.abs(budget)
        });
...
async function submitDeleteProject() {
    try {
        await window.__invoke__('delete_project', { id: currentProjectId });
        document.querySelector('.modal-overlay')?.remove();
        await loadProjects();
    } catch (e) {
